 ...
 */
class Insleads extends BaseController
{
...
//  This is default constructor of the class
    ...
    //    $this->load->model('admin/insleads_model');
       $this->load->model('front/insleads_model');
       $this->isLoggedIn();
    }
    ...
    {   
      ...
      $query =  $this->db->from("campan")->where('seen =', 0)->get();
       $get_tem= $query->result();
        ...
        $this->loadViews("admin/insleads/list", $this->global, NULL , NULL ,'admin');
    ...
    public function addnew()
    {
        ...
        $this->loadViews("admin/insleads/addnew", $this->global, NULL , NULL ,'admin');   
    } 
     ...
     public function insertnow()
     {
         ...
         else
         {
            ...
            if(isset($form_data['next_date']) && $form_data['next_time']){
             ...
             $result = $this->insleads_model->save($insertData);
             ...
             redirect('admin/insleads/addnew');
           }  
    ...
    public function delete()
    {
        ...
        $result = $this->insleads_model->delete($id);
    ...
    {  
        error_reporting(0);
		$list =$this->insleads_model->get_datatables();
        ...
        $adPosition = (isset($utm) && isset($utm['adposition']) && !empty($utm['adposition']))?" | ".$utm['adposition']:'';

        // temp data 
        $userData = array();
        if(isset($currentObj->tempData) && !empty($currentObj->tempData))  {
            $userData = json_decode($currentObj->tempData);
        }  
        $userName = isset($userData->name)?$userData->name:'';
        $userPhone = (isset($userData->mobile) && !empty($userData->mobile))?' | <span class="badge bg-success">'.$userData->mobile."</span>":'';
         ...
         if(isset($category) && !empty($category)){
            ...
            $row[]= $currentObj->device_ip;
            $row[]= $currentObj->camp_id;
            $row[]= $currentObj->type;
            $row[]= $currentObj->keyword;
            $row[]=$currentObj->device;
            $row[]=$currentObj->network.$adPosition;
            $row[]=(isset($currentObj->tempData) && $currentObj->tempData != null)?$userName.$userPhone:'<span class="badge bg-success">No</span>';
            $row[] = $date_at.$new;
            $row[] = '<a class="btn btn-sm btn-info " style="margin-bottom: 4px;" href="'.base_url().'admin/insleads/view/'.$currentObj->id.'" title="edit"  data_id="'.$currentObj->id.'" ><i class="fa fa-eye"></i></a>&nbsp;&nbsp; <a class="btn btn-sm btn-danger deletebtn" style="margin-bottom: 4px;" href="javascript:void(0)" title="delete"  data_id="'.$currentObj->id.'" ><i class="fa fa-trash"></i></a> ';
            $data[] = $row;
        ...
        $output = array(
                        ...
                        "recordsTotal" => $this->insleads_model->count_all(),
                        "recordsFiltered" => $this->insleads_model->count_filtered(),
                        "data" => $data,
                );
    ...
    public function view($id = NULL)
    {
        $w['id'] = $id;
        $w['seen'] = 1;
         $this->insleads_model->save($w);
        $data['view_data'] = $this->insleads_model->find($id);
       
        $category = $this->Case_category_model->all();
        $data['category_id'] = $category;

        $this->global['pageTitle'] = 'Insaaf99 : edit Client';
        $this->loadViews("admin/insleads/view", $this->global, $data , NULL ,'admin');    
     ...
     public function update()
     {
         ...
         else
         {
	        ...
	        $getDataById =  $this->insleads_model->find($form_data['id']);
            ...
            $result = $this->insleads_model->save($updatetData);
     ...
     public function newUpdate(){
        ...
        $w['id']=$_POST['id'];
        $w['field']="additional";
        $additional= $this->insleads_model->finddynamic($w);
        ...
        $updateData['additional']=json_encode($additional);
         ...
         $result =  $this->insleads_model->save($updateData);
        if($result > 0){
            echo 1;
            exit();
        }
    ...
    public function seenAuto()
    {
        ...
        if($_POST['id'] =="seenAll"){
            ...
            $sql .= "SET seen=1 ";
            $sql .= "WHERE seen=0 ";
            $result =  $this->insleads_model->rawQuery($sql);
            echo 1;
            exit();
        }else{
            $updateData['id'] = $_POST['id'];
