...
class Certificate_verify extends BaseController
{
    ...
    public function index()
    {
		...
		if($success===true)
		{
			...
			$data = array(
             ...
             $sql1='';
			 $sql1 .= "select p.*,p.id as paymentId , d.*,d.id as d_id,c.*,p.payment_status,p.id as pay_id from z_payment as p ";
			 $sql1 .="JOIN docx_certificate as d ON d.payment_id = p.id "; 
			 $sql1 .="JOIN clint as c ON c.id = p.user_id "; 
		...
		$client=$this->client_model->find($orderData->user_id);
		$certificate=$categoryData[0];
		$payment=$orderData;

		 certificate_request_admin_message($client,$certificate,$payment);// for Admin

		 certificate_request_client_message($client,$certificate,$payment);// for Client

		$addNotiToAdmin=array();
		$addNotiToAdmin['user_type']=1;// for Admin
		$addNotiToAdmin['user_id']=2;
		$addNotiToAdmin['subject']="New Certificate request ".$orderUserName;
		...
		$this->session->set_flashdata('success', 'Your document request sent successfully');
		redirect(base_url('client/dashboard'));
			...
			$this->session->set_flashdata('error', 'Failed your payment please pay and book now');
			redirect(base_url('client/documentation'));
		}
    }
