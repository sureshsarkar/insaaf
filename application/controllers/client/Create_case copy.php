...
// exit();
        ...
        */
         $schedule_time = array(); $arrayDate = array(); $response = ''; 
         $dateTime = date('Y-m-d H:i:s');
        $i=31;
        $futureDateTime = strtotime(date("Y-m-d H:i:s", strtotime($dateTime)) . " +".$i."days");
        $futureDateTime =date("Y-m-d H:i:s",$futureDateTime);
        $sql ="";
         $sql .="SELECT `meeting_time` FROM `slot` WHERE `meeting_time` BETWEEN '$dateTime' AND '$futureDateTime'";
        $schedule_time =$this->Slot_model->rawQuery($sql);
        if (!empty($schedule_time)) {
         ...
         foreach($schedule_time as $schedule){
            ...
            $PostDate = $this->input->post('schedule_date');
            // pre($schedule->meeting_time);
            // exit();
            $arrayDate = date("d M Y",strtotime($schedule->meeting_time));
            $arrayTime = date("h:i A",strtotime($schedule->meeting_time));

        //    exit();
            // $booked = check_slot($this->session->userdata('ses_lawyer_id'), date('Y-m-d', strtotime($Date)), $schedule); 
            if ($PostDate == $arrayDate) {
                // pre($arrayDate);
                // pre($PostDate);
                //     exit();
                $disabled = 'disabled'; 
            ...
            // $response .= '
            //     <label class="btn btn-outline-primary des_btn_colr_fjjjg '.$active.' '.$disabled.'">
            //        <input type="radio" name="schedule_time" value="'.$arrayTime.'" '.$checked.'> '.$arrayTime.'
            //     </label>
            // ';
