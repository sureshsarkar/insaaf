...
class Meeting_list extends BaseController
{
    ...
    {  
        ...
        $this->loadViews("sub_admin/meeting_list/list", $this->global, NULL , NULL ,'sub_admin');
    ...
    public function view($id = NULL)
    {
       // update seen status
        ...
        $this->loadViews("sub_admin/meeting_list/view", $this->global, $data , NULL,'sub_admin');    
    ...
    {  
        ...
        foreach ($list as $currentObj) {
            ...
            $row[]=$status1;
            $row[]=$new;
            $row[] = '<a class="btn btn-sm btn-info" style="margin: 4px 0px" href="'.base_url().'sub_admin/meeting_list/view/'.$currentObj->id.'" title="View" ><i class="fa fa-eye"></i></a>  ';
         ...
         $result= $this->db->update('slot',$data,'lawyer_id="'.$form_data['lawyer_id'].'" AND slot_status=1');
            ...
            if(!empty($result))
            {
                 echo json_encode(array(
                     'status' => 'true',
                     'reload' => base_url('sub_admin/meeting_list/index/' .$form_data['lawyer_id'])
                 ));
            }
            else
            { 
    ...
    //    exit();
          ...
          $result= $this->db->update('query',$data,'lawyer_id="'.$form_data['lawyer_id'].'"');
            ...
            if(!empty($result))
            {
                 echo json_encode(array(
                     'status' => 'true',
                     'reload' => base_url('sub_admin/Text_query/index/' .$form_data['lawyer_id'])
                 ));
            }
            else
            { 
