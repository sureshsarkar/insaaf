...
class Index extends MailController
{
    ...
    public function index()
    {
        ...
        // headers
          ...
          if($data->action == 'get_all_user' || $data->action == 'send_otp' || $data->action == 'set_video_callid' || $data->action == 'get_video_callid' || $data->action == 'find_dynamic' || $data->action == 'save' || $data->action == 'delete'|| $data->action == 'Get_client_details' || $data->action == 'updateMeetingStauts' || $data->action == 'raw_query' || $data->action == 'upload_file' || $data->action == 'otp_email' || $data->action == 'update_schedule' || $data->action == 'slot_booking_msg' || $data->action == 'call_push_notification' || $data->action == 'first_query_admin_notification' || $data->action == 'certificate_alert_msg' || $data->action == 'slot_booking_alert_msg' || $data->action == 'welcome_msg' || $data->action == 'get_block_date' || $data->action == 'get_all_news' || $data->action == 'get_token' ){
            $apiFn    =  $data->action; 
    ...
    function get_token($data){
      $this->load->model('admin/slot_model');
        ...
        $w['id'] = $data->slot_id;
        $w['field'] = "id,accessToken";
        $accessToken = $this->slot_model->findDynamic($w);
        $rData['success'] = 1;
        $rData['returnVal'] = $accessToken;
        return $rData;
...
$certificate =$this->certificate_model->rawQuery("SELECT c.*, sc.sub_sub_category_name as name, sc.sub_sub_category_name
FROM docx_certificate as c LEFT JOIN `sub_sub_category` sc ON sc.id = c.doc_id WHERE c.id =
'".$data->certificate_id."'");
...
// get client details
$client =$this->client_model->find($data->user_id);
$slot =$this->slot_model->rawQuery("SELECT s.*,c.case_description, cc.name FROM slot as s LEFT JOIN `cases` c ON c.id =
s.case_id LEFT JOIN case_category as cc ON cc.id = c.case_category_id WHERE s.id = '".$data->slot_id."'");
$payment =$this->payment_model->find($data->payment_id);
...
$arr = array('user_id' => $tempUserID,'user_type' => $w['type'],'msg_title' => $name. '
('.ucfirst($w['type']).')','msg_body' => $w['msg'],'navi_screen' => 'query');
