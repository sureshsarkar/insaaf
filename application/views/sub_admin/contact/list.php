...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ...
            <?=(isset($_GET['type']) && !empty($_GET['type']))?'PPC':'Contact';?> List
            ...
            <div class="col-xs-12">
                <div class="box p-2">
                    <div class="d-flex flex-row m-2">
                        <button type="button" class="btn btn-success clickPPC" data-value="ppc">PPC Query</button>
                        <button type="button" class="btn btn-warning clickPPC" data-value="">Contact Query</button>
                    </div>
                        ...
                        <h3 class="box-title">Filter Query List
                            <span style="margin-left:10px;">
                                <select id="shortBtn">
                                    ...
                                    <option value="ppc"
                                        <?= (isset($_GET['type']) && $_GET['type'] == 'ppc' )?"selected":''?>>PPC
                                    </option>
                                </select>
                            </span>
                            <span style="margin-left:10px;">
...
jQuery(document).ready(function() {
    ...
    jQuery(document).on("click", ".deletebtn", function() {
        ...
        currentRow = $(this);
        hitURL = "<?php echo base_url() ?>sub_admin/contact/delete";
...
$(document).ready(function() {
    $("#shortBtn").change(function() {
        ...
        url = url + 'sub_admin/contact?type=' + sortBy;
...
function _fnGetTableData() {
    ...
    table = $('#example').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('sub_admin/contact/ajax_list')?>" + getcontact_data,
            "type": "POST"
        },
...
jQuery(document).ready(function() {
    ...
    jQuery(document).on("click", ".deletByCheck", function() {
        ...
        if (allVals.length > 0) {
            var hitURL = "<?php echo base_url() ?>sub_admin/contact/deleteByCheck";
            $.ajax({
                type: 'POST',
                url: hitURL,
                data: {
...
$(document).ready(function() {
    $(".find_by_date").click(function() {
        ...
        var url = "<?php echo base_url();?>";
        url = url + 'sub_admin/contact?type=daterangefilter&from_date=' + from_date + '&to_date=' +
            to_date;
        window.location.href = url;
    });
});
