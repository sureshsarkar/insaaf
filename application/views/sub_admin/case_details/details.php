        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12 m-auto">
                    <h3><a href="<?= base_url('sub_admin/case_details') ?>"><i class="bi bi-arrow-left"></i></a> Case
                        Details</h3>
                </div>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <div class="box-cart mt-2">
                                                <div class="box-head"><i class="bi bi-person"></i>Change Date & Time
                                                </div>
                                                <form>
                ...
                <div class="col-md-6 m-auto">
                    <div class="for_client__">
                        <div class="for_imffhd">
                            <h4 class="text-uppercase"><b>Assign Lawyer</b></h4>
                        </div>
                        <div class="table-responsive">
                                ...
                                <div class="box-body px-2">
                                    <select class="form-control" id="reAssignLawyer">
                                        ...
                                        <?php foreach($allLawyers as $k=> $v){
                                            ...
                                            ?>
                                        <option value="<?= $v->id ?>">
                                            <?= $v->fname." ".$v->lname."/".$v->mobile."/".$v->city?></option>
                                        <?php }} ?>
                                    </select>
                                </div>
                            </div>
                            ...
                            <label><input type="radio" name="lawyerId" class="checkBox" value="<?= $v->id ?>"> &nbsp;
                                <?= $v->fname." ".$v->lname." / ".$v->mobile." /".$v->city ?> </label><br />
                            <?php }}else{
...
$(document).ready(function() {
    ...
    $(".changeDateTime").click(function(e) {
        ...
        var url = "<?php echo base_url('sub_admin/Case_details/update_date_time')?>";
        $.ajax({
            type: "POST",
            url: url,
            data: {
    ...
    $(".update_meeting").click(function() {
        var case_id = "<?= $case_data->id?>";
        var url = "<?php echo base_url('sub_admin/Case_details/update_meeting')?>";
        $.ajax({
            type: "POST",
            url: url,
            data: {
    ...
    $('#submitLink').submit(function(e) {
        ...
        var url = "<?php echo base_url('sub_admin/Case_details/teamsLink')?>";
        $.ajax({
            type: "POST",
            url: url,
            data: {
...
function _fnSubmitAjax(val = '', meetingDate = '', meetingTime = '') {
    var url = "<?= base_url("sub_admin/case_details/assign_lawyer") ?>";
    var case_id = "<?= $case_data->id?>";
    $.ajax({
        type: "POST",
        url: url,
