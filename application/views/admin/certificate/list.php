...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            <i class="fa fa-sitemap" aria-hidden="true"></i> Certificate List
            ...
            <div class="col-xs-12">
                <div class="box p-2">
                    <div class="d-flex flex-row m-2">
                        <button type="button" class="btn btn-success clickCertificate" data-value="1">Today</button>
                        <button type="button" class="btn btn-warning clickCertificate" data-value="7">Weekly</button>
                        <button type="button" class="btn btn-info clickCertificate" data-value="30">Monthly</button>
                        <button type="button" class="btn btn-success clickCertificate" data-value="365">Yearly</button>
                    </div>
                        ...
                        <h3 class="box-title">Filter Query List
                            <span style="margin-left:10px;">
                                <select id="shortBtn">
                                    ...
                                    <option value="7"
                                        <?= (isset($_GET['type']) && $_GET['type'] == '7' )?"selected":''?>>Weekly
                                    </option>
                                    <option value="30"
                                        <?= (isset($_GET['type']) && $_GET['type'] == '30' )?"selected":''?>>Monthly
                                    </option>
                                    <option value="365"
                                        <?= (isset($_GET['type']) && $_GET['type'] == '365' )?"selected":''?>>Yearly
                                    </option>
                                </select>
                            </span>
                            <span style="margin-left:10px;">
...
$(document).ready(function() {
    $("#shortBtn").change(function() {
        ...
        url = url + 'admin/certificare/?type=' + sortBy;
...
function _fnGetTableData() {
    ...
    ?>
    var getcertificate_data = "<?php echo  isset($_GET['type'])?'?type='.$_GET['type'].$from_date.$to_date:''?>";
    ...
    table = $('#example').DataTable({
        ...
        "ajax": {
            "url": "<?php echo site_url('admin/certificare/ajax_list')?>" + getcertificate_data,
            "type": "POST"
        },
...
$(document).ready(function() {
    $(".clickCertificate").click(function() {
...
$(document).ready(function() {
    $(".find_by_date").click(function() {
        ...
        var url = "<?php echo base_url();?>";
        url = url + 'admin/certificare/?type=daterangefilter&from_date=' + from_date + '&to_date=' +
            to_date;
        window.location.href = url;
    });
});
