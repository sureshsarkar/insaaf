...
class Insleads_model extends Base_model
{   
    ...
    /* end product list with multiplr where condition */
        ...
        public function filterDataFunction(){
            ...
            if(isset($_GET['type']) && $_GET['type'] == 'today'){
                ...
                $this->db->where('campan.created_at <=', $curentdate);
                $this->db->where('campan.created_at >=', $fromdate);
                
            }
            
            if(isset($_GET['type']) && $_GET['type'] == 'daterangefilter'){
                // $from_date= date('Y-m-d 00:00:00', strtotime($_GET['from_date']));
                // $to_date= date('Y-m-d 00:00:00', strtotime($_GET['to_date']));
                // $this->db->where('campan.created_at <=', $from_date);
                // $this->db->where('campan.created_at >=', $to_date);

                $from_date= date('Y-m-d H:i:s', strtotime($_GET['from_date']));
                $to_date= date('Y-m-d H:i:s', strtotime($_GET['to_date']));
                $this->db->where('campan.created_at BETWEEN "'. $from_date. '" and "'.$to_date.'"');
            }
