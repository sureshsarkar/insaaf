...
class Googlelogin_model extends Base_model
{
    public $table = "googlelogin";
    var $column_order = array(null,'user_google_id','device_type','fname','lname','email','image','created_at'); //set column field database for datatable orderable
    var $column_search = array('user_google_id','device_type','fname','lname','email','image','created_at'); //set column field database for datatable searchable 
    ...
    public function find($id) {
       ...
       // Get  List
        ...
        public function joinQuery(){
            $this->db->join('act_category as ac ',' ac.id = acts.act_type');
            $this->db->join('act_sub_category as asc ',' asc.id = acts.sub_category_id');
        }

        public function action_condition(){
            // action type
            if(isset($_GET['type']) && $_GET['type'] == 'open'){
                $this->db->where('acts.status',0);   
            }
            if(isset($_GET['type']) && $_GET['type'] == 'close'){
                $this->db->where('acts.status',1);   
            }
